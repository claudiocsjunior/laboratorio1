...
void Funcionalidades::executarFuncionalidades(){
	...
	do{
		...
		if(menuPrinc != 0 && menuPrinc != 3 && Empresa::m_totalEmpresas > 0){
			do{
...
void Funcionalidades::funcionalidadeMediaFuncionarioEmpresa(){
	...
	if(Empresa::m_totalEmpresas != 0){
		cout << "FUNCIONÁRIOS:  " << Funcionario::m_totalFunc << endl;
		cout << "EMPRESAS:  " << Empresa::m_totalEmpresas << endl;
		double teste = Funcionario::m_totalFunc/Empresa::m_totalEmpresas;
		cout << "A MÉDIA DE FUNCIONÁRIOS POR EMPRESA É " << teste << endl;
	}
	else
	{
...
void Funcionalidades::listarFuncionariosEmExperiencia(EmpresaConjunto &empresaConjunto, int empresaLogada){
	/** @brief Chama o método listar os funcionários no periodo de experiência*/
